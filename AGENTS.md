...
- **After every change, check if `AGENTS.md` or `README.md` need updating** (new files, changed architecture, new conventions, modified project structure). Keep project documentation always up to date.
...
- **Blog** (`/blog`, `/blog/[slug]`, `/en/blog`, `/en/blog/[slug]`): Static bilingual MDX articles for SEO acquisition
- **Language toggle**: FR/EN on every page

### Data Reference

All fiscal data is centralized in `docs/tax-data-2025.md`. Codex MUST reference this file for any data displayed on the site. No numbers from memory.
...
content/
  blog/
    fr/                   # French MDX blog articles
    en/                   # English MDX blog articles, same slugs as FR
components/
...
lib/
  blog.ts                 # Blog registry, metadata, related modules, JSON-LD helpers
  i18n/
...
mdx-components.tsx        # Shared MDX typography and table rendering
...
- **Blog articles must use `docs/tax-data-2025.md` as the default source of truth.** If an article includes a number not present there, it must cite an explicit source in the article metadata and body.
- **Blog slugs are shared across FR and EN.** Add or remove a post in both `content/blog/fr` and `content/blog/en`, then register both versions in `lib/blog.ts`.
